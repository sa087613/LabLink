//FIX MOBILE APPLICATION - TAKES UP
"use client";
...
export function TextFlippingBoard({
  ...
  >
    {watermark && (
      <motion.span
        className="pointer-events-none absolute inset-0 z-0 flex select-none items-center justify-center overflow-visible whitespace-nowrap text-[14vw] font-bold tracking-wide text-amber-600 md:text-[8rem] dark:text-amber-400"
        initial={{ opacity: 0, y: -20 }}
    ...
    <div
      className={cn(
        "relative z-10 rounded-xl bg-neutral-100 p-2 shadow-xl md:rounded-2xl md:p-4 dark:bg-neutral-900 dark:shadow-[0_20px_70px_-15px_rgba(0,0,0,0.6)]",
        className,
      )}
    >
      <div
          ...
          row.map((cell, c) =>
            cell.type === "color" ? (
              ...
              <FlapCell
                key={`${r}-${c}`}
                target={cell.value}
                delay={stagger ? c * colDelay + r * rowDelay : 0}
                stepMs={stepMs}
